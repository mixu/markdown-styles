...
module.exports = function(list, options) {
  ...
  list.files.forEach(function(item) {
    ...
    forEachFileInDirSync(options.partials, function(content, basename){
      partials[basename] = content.toString();
    });

    var helpers = Object.create(null);
    forEachFilePathInDirSync(options.helpers, function(filePath, basename){
      helpers[basename] = require(path.resolve(filePath));
    });
...
function forEachFileInDirSync(dirPath, iterator) {
  forEachFilePathInDirSync(dirPath, function(filePath, basename){
    iterator(fs.readFileSync(filePath).toString(), basename);
  });
}

function forEachFilePathInDirSync(dirPath, iterator) {
  if (dirPath && fs.existsSync(dirPath) && fs.statSync(dirPath).isDirectory()) {
    fs.readdirSync(dirPath).forEach(function(file) {
      var filePath = path.join(dirPath, file);
      if (fs.statSync(filePath).isFile()) {
        var basename = path.basename(file, path.extname(file));
        iterator(filePath, basename);
      }
    });
  }
}
