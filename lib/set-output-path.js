...
module.exports = function(opts) {
  return pi.map(function(item) {
    ...
    var targetDir = opts.output + path.sep + 'json' + path.sep + 'files' + path.sep;

    if (opts.isSingleFile) {
      outputDir = path.normalize(item.path.replace(opts.input, targetDir));
    } else {
      outputDir = path.normalize(path.dirname(item.path).replace(opts.input, targetDir));
    }
    var extension = path.extname(item.path);
    // path: full path to the output file
    item.path = path.normalize(outputDir + path.sep + path.basename(item.path, extension) + '.json');
