...
Generator.prototype.sort = function() {
  ...
  var full = config.order.map(function(infile, index) {
    var fullname = path.normalize(config.input+'/'+infile+'.md');
...
Generator.prototype.each = function(callback, marked) {
  var self = this;
  this.files.forEach(function(sourceFile) {
    var targetFile = self.output + path.sep + path.basename(sourceFile, '.md') + '.html',
